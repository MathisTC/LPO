<template>
    <div v-if="user.loggedIn" class="center-div">
      ...
      <v-row align="start">
        <v-col>
          ...
          <br>
          <v-textarea  label="Instruction de l'eco-geste" rows="3" required auto-grow v-model="texte" />
          <br>
  ...
  import { JeuEcogeste } from "../../utils/etapeCreator.js"
  ...
  export default {
    ...
    methods: {
      ...
      async createEtape () {
        var ecogeste = new JeuEcogeste(JSON.parse(JSON.stringify(this.parcour)).etapes.length + 1, this.titre, '', this.texte, this.poids)
        try {
