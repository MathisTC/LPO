<template>
  <div v-if="user.loggedIn" class="center-div">
    <h2 align="center">
      Création d'un jeu "Le Saviez-vous ?"
    </h2>
    <br>
    <v-row align="start">
      <v-col>
        ...
        <br>
        <v-textarea  label="Information" rows="3" required auto-grow v-model="description" />
        <br>
...
import { JeuInfo } from "../../utils/etapeCreator.js"
import { getParcoursContents, addEtapeInParcours } from "../../utils/queries.js"

export default {
  name: "leSaviezVousComponent",
  data() {
    return {
      image: '',
      bytesarray: '',
  ...
  methods: {
    ...
    async createEtape () {
      var information = new JeuInfo(JSON.parse(JSON.stringify(this.parcour)).etapes.length + 1, this.titre, '', this.description, this.poids)
      try {
